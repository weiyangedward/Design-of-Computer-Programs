...
def compile_formula(formula, verbose=False):
    ...
    firstletter = set(re.findall(r'\b([A-Z])[A-Z]', formula)) # make sure to add 'r' because it is using '\b'
    test = ' and '.join([L+'!=0' for L in firstletter])
    ...
    body = ''.join(tokens)
    if firstletter: body = test + ' and ' + body
