...
def subway(**lines):
    ...
    successor = collections.defaultdict(dict) # defaultdict is dict subclass that ignore missing key value
    for line, stops in lines.items():
        stops = stops.split()
        for i in range(len(stops)-1):
            successor[stops[i]][stops[i+1]] = line
            successor[stops[i+1]][stops[i]] = line
    return successor
...
for station, neighbor in boston.items(): print station, neighbor

def ride(here, there, system=boston):
    "Return a path on the subway system from here to there."
    def is_goal(state): return state == there
    def subsuccessor(state): return system[state]
    return shortest_path_search(here, subsuccessor, is_goal)
