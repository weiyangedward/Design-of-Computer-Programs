...
def boggle_words(board, minlength=3):
    ...
    def find_words(path, pre):
        if pre in WORDS and len(pre) >= minlength:
            results.add(pre)
        if pre in PREFIXES:
            for j in neighbors(path[-1],N):
                if board[j] != BORDER and j not in path:
                    find_words(path + [j], pre+board[j])

    results, N = set(), size(board)
    for i,L in enumerate(board):
        if L != BORDER:
            find_words([i], L) # use a path of index avoid visited letters or cycles, this can be replaced by using a visited set. To have a fattened 2D matrix also helps to make code concise
    return results
