...
for l in letters: # add lowercase letters to POINTS with value=0
    POINTS[l] = 0
...
def add_suffixes(hand, pre, start, row, results, anchored=True):
    ...
    if PRE in PREFIXES:
        ...
        if is_letter(sq):
            ...
            for L in hand:
                if L == '_':
                    for C in possibilities: # try all letters C in possibilities, instead of l.upper()from letters
