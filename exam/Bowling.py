...
def bowling(balls):
    ...
    ## bowling([int, ...]) -> int
    return final_score(balls, 1)

def final_score(balls, r):
    """
    recursively count score until the 10th round
    """
    if r == 10: return sum(balls)
    b1, b2, b3 = balls[:3]
    cur_score, b_used = 0, 0    # b_used is from which the next score should start counting from
    if b1+b2 >= 10:
        cur_score = b1+b2+b3
        if b1 == 10: b_used = 1
        else: b_used = 2
        ...
        cur_score = b1+b2
        b_used = 2
    return cur_score + final_score(balls[b_used:], r+1)
